...
/**
 * @deprecated Use GameCoordinator instead.
 * This class is kept for backward compatibility and will be removed in a future version.
 * The new architecture uses EventBus and modular subsystems.
 * @see GameCoordinator
 */
export class Game {
  // 核心系统
  ...
  constructor() {
    ...
    this.playerController = new PlayerController(this.playerAircraft, this.gameScene.scene, this.playerStats);
  ...
  private setupCallbacks(): void {
    ...
    this.powerUpManager.onPowerUpCollected = (type, config) => {
      ...
      if (type === PowerUpType.HEALTH) {
        ...
        console.log(`速度提升: ${config.value}x`);
      } else if (type === PowerUpType.DAMAGE) {
        // 伤害提升 100%
        this.playerStats.setDamageMultiplier(config.value);
        console.log(`伤害提升: ${config.value}x`);
      } else if (type === PowerUpType.MULTISHOT) {
        // 多重射击（3 发）
        this.playerStats.setMultiShotCount(config.value);
        console.log(`多重射击: ${config.value} 发`);
      }
    };
    ...
    this.powerUpManager.onPowerUpExpired = (type) => {
      if (type === PowerUpType.SHIELD) {
        ...
        // 重置多重射击
        this.playerStats.resetMultiShot();
        console.log('多重射击结束');
      }
    };
  ...
  private update(deltaTime: number): void {
    ...
    // 更新 HUD
    this.hud.updateHealth(this.playerHealth.getHealthPercent());
  ...
  private playerFire(): void {
    ...
    const multiShotCount = this.playerStats.getMultiShotCount();
    if (multiShotCount > 1) {
      // 多重射击：发射 multiShotCount 发子弹
      const spreadAngle = 0.3; // 扩大一倍
      ...
      for (let i = 0; i < multiShotCount; i++) {
        ...
        offsetForward.applyAxisAngle(new THREE.Vector3(0, 1, 0), angleOffset);
        this.playerProjectilePool.fire(position.clone(), offsetForward.normalize(), this.playerStats.getDamage());
      }
    } else {
      this.playerProjectilePool.fire(position, forward, this.playerStats.getDamage());
    }
